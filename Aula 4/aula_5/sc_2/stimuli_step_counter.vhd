...
begin
	...
	begin
		...
		--wait for 1*CLK_PERIOD;
			...
			while (to_integer(cnt_value) /= 228) loop
				wait for 1*CLK_PERIOD;
			end loop;
			reset_activate;
		...
		assign_input_words (SPEED'VAL(2));
        while (to_integer(cnt_value) /= 456) loop
            wait for 1*CLK_PERIOD;
        end loop;
        reset_activate; -- Dispara o reset na hora exata em que atinge 456
        wait for 50*CLK_PERIOD;

		assign_input_words (SPEED'VAL(3)); 
        while (cnt_guru_rdy /= '1') loop
            wait for 1*CLK_PERIOD;
        end loop;
        wait for 50*CLK_PERIOD;
